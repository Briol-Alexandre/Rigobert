...
<section class="grid-main" aria-label="histoire">
    ...
    <div class="col-start-6 col-end-11">
        <a href="<?= $images[0]['url'] ?>"
            class="gallery-image cursor-pointer block h-full overflow-hidden group relative">
            <span
    ...
    <div class="col-start-11 col-end-12 flex flex-col gap-4">
        <?php for ($i = 1; $i < count($images); $i++) { ?>
            <a href="<?= $images[$i]['url'] ?>"
                class="gallery-image cursor-pointer block h-1/2 overflow-hidden group relative">
                <span
                    class="opacity-0 group-hover:opacity-100 absolute w-full h-full bg-black/50 flex items-center justify-center transition-all duration-300 font-cormorant text-background text-2xl font-bold">Voir
                    l'image</span>
